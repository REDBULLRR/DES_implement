...
bitset str_to_binary(string aStr);
...
bitset str_to_binary(string aStr)
{
  ...
  if (aStr.size() > MAX_STR_SIZE)  
  {
    ...
    return bitset<1>(string("0")); // 不確定能否這樣寫
  }
  
  string eightBytes[64]="";
  ...
  for(int i=0; i<BYTE_SIZE; i++)
  {
    ...
    // reverse aByte, since the binaries were appended in a reversed order
    reverse(aByte.front(), aByte.back());
